...
prefixes='Before using `elect` or `iterate` commands, type the prefix for the electoral method you want.'
prefect='Prefixes are `hic.` `stv.hare.` `stv.droop.`'
'''
@client.event
async def on_ready():
...
def stvElection(quota):
    ...
    stvmalecandidates+=mcandidateselected
    ...
    print('whatthehell', totalmalecandidates, totalfemalecandidates)
    return printlist
...
def finalStvResults():
    ...
    printlist.append("Male Candidates Elected " +str(stvmalecandidates))
...
#@client.event
    if message.content=='stv.hare.elect':
        for x in range(0, 1):
            await message.channel.send(stvElection(45))
        #finalResults()
        response=finalStvResults()
        await message.channel.send(response)
    if message.content=='stv.droop.elect':
        for x in range(0, 1):
            await message.channel.send(stvElection(34))
        #finalResults()
        response=finalStvResults()
        await message.channel.send(response)
    ...
    if message.content=='hic.iterate':
        for x in range(0, 100):
            runElection()
        response=finalResults()
        await message.channel.send(response)
    if message.content=='stv.hare.iterate':
        for x in range(0, 100):
            stvElection(45)
        response=finalStvResults()
        await message.channel.send(response) 
    if message.content=='stv.droop.iterate':
        for x in range(0, 100):
            stvElection(34)
        response=finalStvResults()
        await message.channel.send(response)
    if message.content=='help':
        ...
        await message.channel.send(response)
        await message.channel.send(prefixes+prefect)
